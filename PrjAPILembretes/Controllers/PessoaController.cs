...
namespace PrjAPILembretes.Controllers
{
    ...
    public class PessoaController:ControllerBase//Passo 3 - Herdar de controllerbase
    {
        ...
        //}
        [HttpPost("addperson")]
        public IActionResult AddPerson(PessoaDTO pessoaDTO)//DTO -> Data Transfer Object
        {
            Pessoa newPerson;
            try
            {
                newPerson = new Pessoa(pessoaDTO.NomePessoa,pessoaDTO.IdadePessoa);
        ...
        [HttpPut("update/{name}")]
        public IActionResult UpdatePerson(PessoaDTO pessoaDTO, string name)
        {
            Pessoa person;
            try
            {
                person = new Pessoa(pessoaDTO.NomePessoa,pessoaDTO.IdadePessoa);
